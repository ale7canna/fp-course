...
-- Empty
myContains :: Eq a => Optional a -> List (Optional a) -> Bool
myContains a Nil = False
myContains a (x :. xs) = (a == x) || (myContains a xs)

seqOptional :: Eq a => List (Optional a) -> Optional (List a)
seqOptional x
  | myContains Empty x = Empty
  | otherwise = Full (map (\(Full x') -> x') x)
